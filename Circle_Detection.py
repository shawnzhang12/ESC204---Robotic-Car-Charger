...
while StepCounter < 200:
    ...
    StepCounter += 1
#Example code for detecting port in one image
img = cv2.imread('40cm.jpg')
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
gray = cv2.GaussianBlur(gray, (9, 9), 4, 4)
retval, threshold = cv2.threshold(gray, 5, 255, cv2.THRESH_BINARY)
cv2.imwrite("40cm_binarization.png",threshold)
circles = cv2.HoughCircles(threshold, cv2.HOUGH_GRADIENT, 1, 400, param1=500, param2=15, minRadius=20, maxRadius=100)
#print(circles)
#print(circles[0][0][1])
for i in circles[0]:
    cv2.rectangle(img,(round(i[0]-i[2]),round(i[1]+i[2])),(round(i[0]+i[2]),round(i[1]-i[2])),(0,0,255),2)
cv2.imwrite("40cm_hole.png",img)
